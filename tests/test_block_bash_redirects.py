...
from block_bash_redirects import (  # noqa: E402
    HabitEntry,
    _BAD_HABITS,
    _load_config,
    check_command,
...
def test_disabled_suppresses_rule(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_disabled_does_not_affect_other_rules(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    config = tmp_path / ".claude" / "rubber-band.json"
    config.parent.mkdir()
    config.write_text(json.dumps({"disabled": ["cat"]}))
    monkeypatch.setenv("PWD", str(tmp_path))
...
def test_extra_habits_blocks_custom_pattern(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    config = tmp_path / ".claude" / "rubber-band.json"
    config.parent.mkdir()
...
def test_extra_habits_invalid_regex_skipped(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    config = tmp_path / ".claude" / "rubber-band.json"
    config.parent.mkdir()
    config.write_text(json.dumps({
...
def test_project_config_extends_global(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    home = tmp_path / "home"
    project = tmp_path / "project"
    (home / ".claude").mkdir(parents=True)
    (project / ".claude").mkdir(parents=True)
    (home / ".claude" / "rubber-band.json").write_text(json.dumps({
        "extra_habits": [{"pattern": r"\bpip\b", "reason": "Use uv add."}],
    }))
    (project / ".claude" / "rubber-band.json").write_text(json.dumps({
        "disabled": ["cat"],
    }))
